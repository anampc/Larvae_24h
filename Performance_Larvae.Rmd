...
Well<- ggplot(data, aes (Treatment, nmol.min)) +
  ...
  scale_y_continuous(limits = c(0, 0.01),
                     #   breaks = seq(0,1,0.2),  
                     #   expand = c(0.01, 0.01),
                     name=("O2 [nmol/min] per well")) +
  ggthe_bw +
  facet_grid(~Timepoint)
Well

Well_6<- ggplot(data, aes (Treatment, nmol.min/6)) +
  geom_boxplot ()+
  stat_summary(fun.data = "mean_cl_boot", 
               geom = "errorbar", width = 0.2, color="blue")+
  stat_summary(fun=mean, geom="point",  color="blue") + 
  ...
  scale_y_continuous(limits = c(0, 0.0018),
                     #   breaks = seq(0,1,0.2),  
                     #   expand = c(0.01, 0.01),
                     name=("O2 [nmol/min] per larve (6 in each well)")) +
  ggthe_bw +
  facet_grid(~Timepoint)
Well_6
...
Larvae<- ggplot(data, aes (Treatment, nmol.larva.min)) +
  ...
  scale_y_continuous(#limits = c(0, 1),
                      #   breaks = seq(0,1,0.2),  
                      #   expand = c(0.01, 0.01),
                         name=("O2 [nmol/min/larva] corrected")) +
  ggthe_bw +
  facet_grid(~Timepoint)
Larvae
...
## model 1: LMER for exposure well
  ...
  fit1<-lmerTest::lmer(log(nmol.min) ~Treatment + (1|Replicate.vial),
              data=data[data$Timepoint=="Exposure",])
  anova(fit1)
  ranova(fit1)
  summary(fit1)
